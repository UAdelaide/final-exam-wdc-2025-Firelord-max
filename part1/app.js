  ...
  try {
    ...
    if (rows_3[0].count === 0) {
      await db.execute(`
        ...
        ((SELECT request_id FROM WalkRequests WHERE request_id = 2), (SELECT walker_id FROM Users WHERE name = 'bobwalker'),(SELECT owner_id FROM Users WHERE name = alice123'), 5, 'Slow dog, but cute');
      `);
  } catch (err) {
