  ...
  try {
    ...
    if (rows_3[0].count === 0) {
      await db.execute(`
        ...
        ((SELECT request_id FROM WalkRequests WHERE request_id = 1), (SELECT walker_id FROM Users WHERE name = 'bobwalker'),(SELECT owner_id FROM Users WHERE name = alice123'), 5, 'Slow dog, but cute', 'Parklands', 'open'),
        ((SELECT dog_id FROM Dogs WHERE name = 'Bella'), '2025-06-10 09:30:00', 45, 'Beachside Ave', 'accepted');
      `);
  } catch (err) {
